...
const myDmuMarkLocalOnly = (data) => myDmuBooleanConfig(data, 'MyDMU_LocalMarkV2', false);

const myDmuMarkEnabled = (data, key) =>
  myDmuBooleanConfig(data, 'MyDMU_AutoMarkV2', false) && myDmuBooleanConfig(data, key, false);
...
const myDmuApplyP1PoisonMarkers = (data, force = false) => {
  if (!myDmuMarkEnabled(data, 'MyDMU_P1PoisonMarkV2'))
    return false;
...
const myDmuClearMarks = (data) => {
  ...
  if (!myDmuBooleanConfig(data, 'MyDMU_AutoMarkV2', false))
    return;
  const fl = myDmuFl(data);
...
const myDmuApplyP2Round = (data, round) => {
  if (!myDmuMarkEnabled(data, 'MyDMU_P2TowerMarkV2'))
    return false;
...
const myDmuApplyP3MahjongMarkers = (data) => {
  if (!myDmuMarkEnabled(data, 'MyDMU_P3MahjongMarkV2'))
    return false;
  const state = data.myDmuP3Mahjong;
  if (state?.marked)
    return true;
...
const myDmuTryApplyP3TargetMarkers = (data) => {
  if (!myDmuMarkEnabled(data, 'MyDMU_P3TargetMarkV2'))
    return false;
...
const myDmuTrySendP4LongAttack12Chat = (data) => {
  if (!myDmuMarkEnabled(data, 'MyDMU_P4BuffMarkV2'))
    return true;
...
function myDmuProcessP4MarkTiming(data, source = 'process') {
  if (data.myDmuPhase !== 'p4' || !myDmuMarkEnabled(data, 'MyDMU_P4BuffMarkV2'))
    return true;
...
Options.Triggers.push({
  ...
  config: [
    ...
    {
      id: 'MyDMU_P3MahjongMarkV2',
      name: { en: '自用：P3 麻将标点' },
      type: 'checkbox',
      default: false,
    },
    {
      id: 'MyDMU_P3TargetMarkV2',
  ...
  triggers: [
    ...
    },
    {
      ...
      preRun: (data, matches) => {
        ...
        data.myDmuP1PoisonMarkerSignature = undefined;
        if (data.myDmuP1PoisonTargets.length === 0 && myDmuMarkEnabled(data, 'MyDMU_P1PoisonMarkV2'))
          myDmuScheduleClearMarks(data, 'p1Poison', 0.2, (data) =>
            data.myDmuP1PoisonTargets.length === 0 && myDmuMarkEnabled(data, 'MyDMU_P1PoisonMarkV2'));
      },
    },
    {
    ...
    },
    {
      ...
      run: (data) => {
        ...
        if (round >= 8 && myDmuMarkEnabled(data, 'MyDMU_P2TowerMarkV2'))
          myDmuScheduleClearMarks(data, 'p2Tower', 1.2, (data) =>
            (data.myDmuP2AbilityRound ?? 0) >= 8 && myDmuMarkEnabled(data, 'MyDMU_P2TowerMarkV2'));
      },
    },
    {
      id: '绝妖星 P3 水火Debuff',
